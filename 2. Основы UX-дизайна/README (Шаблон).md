...
Если первый модуль помог понять, кто такой UX/UI-дизайнер и какие инструменты он использует, то теперь ты углубишься в самое сердце профессии — исследования. Любой интерфейс начинается не с рисования кнопок, а с понимания кто будет пользоваться продуктом и как. UX-исследования помогают изучить поведение людей, их цели, проблемы и ожидания, а затем превратить эти знания в решения.
...
Разберешься, как пользователь взаимодействует с сервисом на каждом этапе и выявить точки боли. 
...
Научишься описывать и визуализировать типичного пользователя, чтобы проектировать интерфейс под его потребности.

- Составь одну User Persona для выбранного сервиса.
- Включи: фото (можно взять из фотостоков), демографию, цели, боли, поведение.
- Сделай карточку в Notion или Figma.

В итоге ты получишь карточку персоны с ключевыми характеристиками пользователя

#### Задание 3. User Flow
В этом задании нужно отобразить последовательность шагов пользователя в сервисе и выявить узкие места

- Нарисуй flow для ключевого сценария (например: «Заказать пиццу через приложение»).
...
А здесь ты научишься превращать сценарий в каркасные прототипы экранов.

- На основе User Flow создай 2–3 экрана в Figma в виде wireframe (черно-белые каркасы без визуала).

И после задания у тебя будет набор wireframes ключевых экранов продукта.
...
Твоя задача проверить, как AI помогает строить карту пути пользователя.
...
Тебе нужно синтезировать данные в собирательный образ, а именно:
